...
export class CrSidebar extends LitElement {
    ...
    render() {
        return html`
        <button @click=${this._toggle}  > ${!this.open ? (`🛍️${String(this.cartService?.cartItems.length)}`) : '🛍️'
            }
        </button>
        <div>
       
        <h3>Consumer data: <code>${JSON.stringify(this.cartService?.cartItems)}</code></h3>
        </div>
        `;
    }
}
